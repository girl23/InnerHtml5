	...
	zyMedia.MediaPlayer.prototype = {
		...
		setPlayerSize: function(width, height) {
			...
			if (t.enableAutoSize) {
				...
				t.height = parseInt(_W * 2/5);//parseInt(_W / t.options.aspectRation)
			}
		...
		exitOrEnterFullScreenReturn : function(fun){
			var t = this;
			if(zyMedia.features.isiOS)
				return callback(fun,{'Success':true,'Message':'退出全屏成功','Data':fun});
			else
				return {'Success':true,'Message':'退出全屏成功','Data':fun};
		},
		...
		buildFullscreen: function() {
			...
			t.fullscreenBtn.addEventListener('click', function() {
				...
				if ((zyMedia.features.nativeFullscreenPrefix != '-' && isInFullScreenMode()) || t.isFullScreen) {
					t.exitFullScreen()
					t.exitOrEnterFullScreenReturn('exitFullScreen');
				} else {
					t.enterFullScreen()
					t.exitOrEnterFullScreenReturn('enterFullScreen');
				}
			});
		},
